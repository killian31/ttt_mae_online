...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        norm_pix_loss=False,
        drop_path_rate=0.0,
        rotation_prediction: bool = False,
    ):
        super().__init__()
        ...
        if self.head_type == "vit_head":
            ...
            self.classifier_embed = nn.Linear(
                embed_dim, classifier_embed_dim, bias=True
            )

            self.classifier_pos_embed = nn.Parameter(
                torch.zeros(1, num_patches + 1, classifier_embed_dim),
                requires_grad=False,
            )  # fixed sin-cos embedding

            dpr = [
                x.item() for x in torch.linspace(0, drop_path_rate, classifier_depth)
            ]
    ...
    def initialize_weights(self):
        ...
        if self.head_type == "vit_head":
            classifier_pos_embed = get_2d_sincos_pos_embed(
                self.classifier_pos_embed.shape[-1],
                int(self.patch_embed.num_patches**0.5),
                cls_token=True,
            )
            self.classifier_pos_embed.data.copy_(
                torch.from_numpy(classifier_pos_embed).float().unsqueeze(0)
            )
