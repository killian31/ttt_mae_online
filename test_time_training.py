...
def get_args_parser():
    ...
    parser.add_argument(
        "--resume_model", default="", required=True, help="resume from checkpoint"
    )
    parser.add_argument(
        "--resume_finetune", default="", required=True, help="resume from checkpoint"
    )
...
def load_combined_model(args, num_classes: int = 1000):
    if args.model == "mae_vit_small_patch16":
        ...
        assert (
            "mae_vit_large_patch16" in args.model
            or "mae_vit_huge_patch14" in args.model
        )
    ...
    print("Loading backbone checkpoint from", args.resume_model)
    model_checkpoint = torch.load(args.resume_model, map_location="cpu")
    print("Backbone loaded")
    print("Loading head checkpoint from", args.resume_finetune)
    head_checkpoint = torch.load(args.resume_finetune, map_location="cpu")

    if args.head_type == "linear":
        model_checkpoint["model"]["bn.running_mean"] = head_checkpoint["model"][
            "head.0.running_mean"
        ]
        model_checkpoint["model"]["bn.running_var"] = head_checkpoint["model"][
            "head.0.running_var"
        ]
        model_checkpoint["model"]["head.weight"] = head_checkpoint["model"][
            "head.1.weight"
        ]
